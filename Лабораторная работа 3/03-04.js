...
function factAsync(a, callback){
    if(a == 0 || a == 1){
        process.nextTick(() => callback(null, 1));
    }
    else {
        process.nextTick(() => {
            factAsync(a - 1, (err, result) => {
                if (err) callback(err);
                else callback(null, a * result);
            });
        });
    }
}
...
http.createServer(function(request, response){
    ...
    if(route == '/fact' && parm != 'x'){
        if(parm != null && Number.isInteger(parseInt(parm, 10))){
            ...
            factAsync(buf, (err, factResult) => {
                if (err) {
                    response.writeHead(500, {'Content-Type': 'application/json'});
                    response.end(JSON.stringify({error: err.message}));
                } else {
                    response.end(JSON.stringify({k: buf, fact: factResult}));
                }
            });
        }
    }
